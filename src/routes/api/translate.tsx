...
/**
 * Exported so `/v1/translate` calls this rather than restating it.
 *
 ...
 */
export async function translateHandler({ request }: { request: Request }) {
  const id = requestId()
  ...
  if (typeof payload.target !== 'string' || !isOutputLocale(payload.target)) {
    return Response.json(
      {
        error: 'bad_target',
