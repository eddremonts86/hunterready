...
/**
 * Exported so `/v1/cover-letter` calls this rather than restating it.
 *
 ...
 */
export async function coverLetterHandler({ request }: { request: Request }) {
  const id = requestId()
  ...
  if (
    (mayUseProvider ? resolveProvider() : resolveLocalProvider()) === undefined
  ) {
    return Response.json(
      {
