 ...
 *
 * `.js` was added to the extension list when the service worker arrived: `src/pwa/service-worker.js`
 * and the generated `public/sw.js` are named across half a dozen docblocks, and none of them were
 * checked, because the list had only ever needed the extensions this codebase writes source in.
 *
...
const NOT_CLAIMS: ReadonlyMap<string, string> = new Map([
  ...
  [
    'public/sw.js',
    'Generated by `scripts/make-sw.mjs` before every build and gitignored, because its contents are ' +
      "this build's cache version. It exists after `pnpm build` and not in a fresh clone, so a " +
      'literal check would pass or fail depending on whether somebody had run one — which is the ' +
      'opposite of what this file is for. The parity suite asserts the served copy instead.',
  ],
])
...
const PATH_REF =
  /`([A-Za-z0-9][A-Za-z0-9_./-]*\/[A-Za-z0-9_.-]+\.(?:tsx?|mjs|js|yml|json|md))`/g
